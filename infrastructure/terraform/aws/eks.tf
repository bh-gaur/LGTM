...
# 3. IAM Role for Managed Worker Node Group
resource "aws_iam_role" "eks_nodes" {
  name = "lgtm-eks-node-role"
...
# 4. Managed Node Group (EKS Worker Nodes)
resource "aws_eks_node_group" "node_group" {
  cluster_name    = aws_eks_cluster.eks.name
  node_group_name = "lgtm-workers"
